...
const { userAttributes, imageAttributes, spotAttributes } = require('../../utils/attributes.js')
...
router.get('/:spotId', async (req, res) => {
  ...
  // Find the spot with associated Owner, SpotImages, and Reviews
  const spot = await Spot.findOne({
      ...
      include: [
          ...
          {
              model: Review,
              attributes: ["stars"], // Add more fields if needed
          },
      ],
  });

  // Handle case if spot is not found
  if (!spot) {
      return res.status(404).json({
          message: "Spot couldn't be found",
      });
  ...
  if (spotDetails.Reviews && spotDetails.Reviews.length > 0) {
...
router.post('/:spotId/images', requireAuth, async (req, res, next) => {
  ...
  try {
    ...
    // Check if the image URL and preview status are provided
    if (!url || typeof preview === 'undefined') {
      return res.status(400).json({
        message: "Image URL and preview status are required",
      });
    }
    ...
    // Format the response for the newly created image
    ...
    // Send the newly created image as a response
